...
class ClassificationExperiment(object):
    ...
    def __init__(self, model, writer: SummaryWriter, log_interval: int = 250, lr: float = .001, criterion: str = 'NLLLoss', device='cuda'):
        self.model = model
        self.writer = writer
        ...
        self.criterion = instanciate_cls('torch.nn', criterion)
        ...
        for step, (data, target) in enumerate(tqdm(train_loader, leave=False, desc="Running training phase")):
            ...
            if step % self.log_interval == 0 and step > 0:
                self.writer.add_scalar(
                    'avg_loss/train', np.mean(losses), global_step)
